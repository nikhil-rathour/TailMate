...
const Chat = () => {
  ...
  return (
    <div className="bg-navy min-h-screen text-white px-4 py-8">
      <div className="max-w-3xl mx-auto bg-navy/50 rounded-3xl shadow-xl border-2 border-navy/20 overflow-hidden">
        {/* Chat Header */}
        <div className="bg-navy p-4 flex items-center justify-between border-b border-navy/30">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => navigate(-1)}
              className="text-gold hover:text-accent-orange"
            >
            ...
            <div>
              <h2 className="font-bold">{receiverInfo.receiverName}</h2>
              {typing && <p className="text-xs text-softgray">typing...</p>}
            </div>
          </div>
        </div>
        ...
        {receiverInfo.petId && (
          <div className="bg-navy/70 p-3 flex items-center gap-3 border-b border-navy/30">
        ...
        <div className="h-96 overflow-y-auto p-4 flex flex-col gap-3">
          {loading ? (
            ...
            <div className="text-center py-4">
              <p className="text-softgray">No messages yet. Start the conversation!</p>
            </div>
          ) : (
                ...
                >
                  <div 
                    className={`max-w-[70%] rounded-2xl px-4 py-2 ${
                      isSender ? 'bg-gold text-navy' : 'bg-navy/70 text-white'
                    }`}
                  >
                    <p>{msg.message}</p>
                    <p className={`text-xs mt-1 ${isSender ? 'text-navy/70' : 'text-softgray'}`}>
        ...
        {/* Message Input */}
        <div className="bg-navy/70 p-3 flex gap-2">
          <input
            type="text"
            value={newMessage}
            ...
            className="flex-1 bg-navy/50 text-white rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-gold"
          />
          <button
            onClick={handleSendMessage}
            disabled={!newMessage.trim()}
            className="bg-gold hover:bg-accent-orange text-navy rounded-full w-10 h-10 flex items-center justify-center disabled:opacity-50"
          >
