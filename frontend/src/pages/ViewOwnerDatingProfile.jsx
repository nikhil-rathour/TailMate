...
import { FiEdit, FiMapPin, FiUser, FiArrowLeft, FiStar, FiEye, FiHeart, FiMessageCircle } from 'react-icons/fi';
...
const ViewOwnerDatingProfile = () => {
  ...
  return (
    <div className="bg-navy min-h-screen text-white">
      {/* Header */}
      <div className="sticky top-0 z-50 bg-navy/80 backdrop-blur-sm border-b border-gold/20">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate(`/edit-owner-dating-profile/${profile._id}`)}
            className="flex items-center gap-2 text-gold hover:text-accent-orange transition-colors"
          >
            <FiEdit /> Edit
          </motion.button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-white font-medium">{profile.education || 'Not specified'}</p>
                </div>
              </div>
            </motion.div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {profile.pets.map((pet, idx) => (
                    <div key={idx} className="bg-white/5 rounded-xl p-4 flex items-center gap-4">
                      <img 
                        src={pet.image || 'https://via.placeholder.com/64x64/4A5568/FFFFFF?text=Pet'}
                        alt={pet.name}
                        className="w-16 h-16 rounded-full object-cover border-2 border-gold/30"
                        onError={(e) => {
                          e.target.src = 'https://via.placeholder.com/64x64/4A5568/FFFFFF?text=Pet';
                        }}
                      />
                      <div>
                        <p className="font-semibold text-white">{pet.name}</p>
                        <p className="text-white/70 text-sm">{pet.breed}</p>
                        <p className="text-white/50 text-xs">{pet.age} years old • {pet.type}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold text-blue-400">{stats.views}</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold text-red-400">{stats.likes}</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold text-green-400">{stats.matches}</span>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-gold/20"
            >
              <h3 className="text-lg font-semibold text-gold mb-4">Profile Actions</h3>
              <div className="space-y-3">
                ...
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className={`w-full flex items-center justify-center gap-3 py-3 rounded-lg transition-all duration-300 border ${
                    profile.isActive 
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => navigate('/owner-dating')}
                  className="w-full flex items-center justify-center gap-3 py-3 bg-blue-500/20 text-blue-300 rounded-lg hover:bg-blue-500/30 transition-all duration-300 border border-blue-500/30"
                >
                  Browse Profiles
                </motion.button>
              </div>
            ...
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-gold/20"
            >
              <h3 className="text-lg font-semibold text-gold mb-4">Profile Tips</h3>
